...
2. Charger les vocabulaires dans le DEV et dans la PROD :
	...
	2.4 faire une copie de sauvegarde du named graph actuel
	2.5 supprimer ce named graph
	2.6 **BIEN SPECIFIER LE TARGET NAMED GRAPH** : http://vocab.performing-arts.ch/container/context
	2.6 puis uploader le fichier SKOS 
4. Cleaner en local en faisant `npm run clean`
5. Mettre à jour les données depuis la PROD en lançant `npm run fetch`
